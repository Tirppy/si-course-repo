...
const char* modeToShortLabel(ControlMode mode) {
  switch (mode) {
...
void lcdDisplayShowStatus(float measuredTemperatureC,
  ...
  snprintf(line2, sizeof(line2), "%s H%1d F%03u %c", modeToShortLabel(mode),
           heaterOn ? 1 : 0, fanDutyPercent, serialOverrideEnabled ? 'S' : 'P');
